...
module.exports = function(copilot) {
	...
	_.extend(Runner.prototype, {
			...
			this.scenario
				...
				.step('make sure all the data are well prepared.', function () {
					_.each(runner.dataCollections.EventSourceTemplates.data, function (eventSourceTemplate) {
						if (!isSearchOnly(eventSourceTemplate) && eventSourceTemplate.data.configuration && _.isFunction(eventSourceTemplate.data.configuration.url)) {
					...
					_.each(runner.dataCollections.EventSources.data, function (eventSource) {
						if (!isSearchOnly(eventSource) && eventSource.data.configuration && _.isFunction(eventSource.data.configuration)) {
					...
					_.each(runner.dataCollections.ActionTargetTemplates.data, function (actionTargetTemplate) {
						if (!isSearchOnly(actionTargetTemplate) && actionTargetTemplate.data.configuration && _.isFunction(actionTargetTemplate.data.configuration.url)) {
					...
					_.each(runner.dataCollections.ActionTargets.data, function (actionTarget) {
						if (!isSearchOnly(actionTarget) && actionTarget.data.configuration && _.isFunction(actionTarget.data.configuration)) {
					...
					_.each(runner.dataCollections.EventTypes.data, function (eventType) {
						if (!isSearchOnly(eventType) && eventType.data.type && _.isFunction(eventType.data.type)) {
							eventType.data.type = _.bind(eventType.data.type, this)();
						}
					}, this)

					_.each(runner.dataCollections.ActionTypes.data, function (actionType) {
						if (!isSearchOnly(actionType) && actionType.data.type && _.isFunction(actionType.data.type)) {
		...
		_beforeRun: function () {
			...
			this.managers.actionTargetManager = new Manager(this.scenario, this.dataCollections['ActionTargets'], 'action target', 'actionTargets', {
				...
				getUrl: function (item, baseGetUrl) {
					if (item.template && item.template.id) {
						...
						return baseGetUrl + '&allOrganizations';
					}
				},
				extend: function (item) {
					return _.extend(item.data, {
			...
			this.managers.actionTypeManager = new Manager(this.scenario, this.dataCollections['ActionTypes'], 'action type', 'actionTypes', {
				next: this.managers.actionTargetManager,
				extend: function (item) {
					return _.extend(item.data, {
						organizationId: runner.organizationId
					})
			...
			this.managers.eventTypeManager = new Manager(this.scenario, this.dataCollections['EventTypes'], 'event type', 'eventTypes', {
				next: this.managers.eventSourceManager,
				extend: function (item) {
					return _.extend(item.data, {
						organizationId: runner.organizationId
					});
		...
		_createOrganization: function (orgaName) {
			...
			return this.scenario
				...
				.step('check organization created', function (response) {
					...
					console.log('organization created with id: %s'.green, runner.organizationId);
